...
Grid * chargerGrille(char * filename) {
    ...
    if(fich != NULL) {
        ...
        for(i = 0; i < 9; i++) {
            for(j = 0; j < 9; j++) {
                fscanf(fich, "%d %d ", &grille->cell[i][j].valeur, &grille->cell[i][j].estEditable);
            }
        }
        fclose(fich);
        return grille;
...
int choisirGrille(char *filename, char *niveau) {
    ...
    num = rand() % total + 1;
    printf("Debug: Selected random grid number: %d\n", num); // Debug print
    
    switch (c) {
        case '1': 
            sprintf(filename, "Grille F %d", num); 
            strcpy(niveau, "Facile");
            break;
        case '2': 
            sprintf(filename, "Grille I %d", num); 
            strcpy(niveau, "Intermediaire");
            break;
        case '3': 
            sprintf(filename, "Grille D %d", num); 
            strcpy(niveau, "Difficile");
            break;
    }

    printf("Debug: Final filename: %s, Niveau: %s\n", filename, niveau); // Debug print
    return 1;
}
